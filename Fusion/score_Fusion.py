...
def load_score_file(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in tqdm(f, desc=f"Reading {file_path}"):
            ...
            if len(parts) == 6:
                spk_id, utt_id, _, label, score, _ = parts
...
def compute_all_eers(fused_scores):
    ...
    sasv_eer = compute_eer(fused_scores, ["target"])
    sv_scores = [x for x in fused_scores if x[0] in ["target", "nontarget"]]
    spf_scores = [x for x in fused_scores if x[0] in ["target", "spoof"]]

    sv_eer = compute_eer(sv_scores, ["target"])
    spf_eer = compute_eer(spf_scores, ["target"])
    return sasv_eer, sv_eer, spf_eer


# Example usage:

ASV_dev_file = "Decision_Fusion/output/cosine_scores_dev.txt"
prosody_dev_file = "Decision_Fusion/output/prosody_cosine_dev.txt"
ASV_eval_file = "Decision_Fusion/output/cosine_scores_eval.txt"
prosody_eval_file = "Decision_Fusion/output/prosody_cosine_eval.txt"
...
# EERs
sasv_eer, sv_eer, spf_eer = compute_all_eers(fused_eval_scores)

print(f"SASV EER: {sasv_eer*100:.2f}%")
print(f"SV   EER: {sv_eer*100:.2f}%")
print(f"SPF  EER: {spf_eer*100:.2f}%")
...
#     grid = GridSearchCV(base_model, param_grid, scoring='roc_auc', cv=5, verbose=1, n_jobs=-1)
#     grid.fit(X_scaled, y)
